...
IPAddress LocalIP(HA_SUBNET0, HA_SUBNET1, HA_SUBNET2,GAMMA_NODE);
...
float Temp_degF;
float Humidity_pct;
const float Temp_offset_degF = -1.2f; /* Tuned to align with ALPHA */
const float Humidity_offset_pct = -2.7f; /* Tuned to align with ALPHA */
...
void MQTTTaskUpdate(void){
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (!MQTT_client.connected()) {
      if (MQTT_client.connect("GAMMA")) {
        Serial.println("MQTT Connected");
      } 
    }

    if (MQTT_client.connected()){
      MQTT_client.publish("GAMMA/Temp_F",dtostrf(Temp_degF, 3, 2, tempStr));
      MQTT_client.publish("GAMMA/Humidity_Pct",dtostrf(Humidity_pct, 3, 2, humidStr));
      MQTT_client.loop();
    }
